<?php
    ...
    function lenConvert(int $from, int $to, float $fromVal, $weightUnits){
        if(is_int($from)&& is_int($to)&& is_float($fromVal)){

            $fark = $from - $to;
            $result = $fromVal * pow(10, $fark);  //pow = üst alma, pow(taban, üs)
            $result = strval($fromVal).$weightUnits[$from - 1].'-->'.$result.$weightUnits[$to-1]; //?? Eleman seçmek için
            return $result;
        }
    ...
    <body>
        ...
        <nav class="navbar navbar-expand-lg bg-light">
            <div class="container-fluid">
                <a class="navbar-brand" >Unit Conventor</a>
                        ...
                        <?php
                            if(isset($_GET['fromBtn'])){
                                echo '
                                    ...
                                    <div class="col-md-12 m-2">
                                        <p> '.lenConvert($from, $to, $fromVal, $weightUnits).' </p>
                                    </div>
                                ';
                            }
                        ?>
